...
resource "aws_lambda_function" "wellknown" {
  ...
  function_name    = "${var.project_name}-${var.environment}-wellknown"
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-wellknown"
  }
}
...
resource "aws_lambda_function" "jwks" {
  filename         = data.archive_file.lambda_zip.output_path
  function_name    = "${var.project_name}-${var.environment}-jwks"
  role            = aws_iam_role.lambda_exec.arn
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-jwks"
  }
}
...
resource "aws_lambda_function" "token" {
  filename         = data.archive_file.lambda_zip.output_path
  function_name    = "${var.project_name}-${var.environment}-token"
  role            = aws_iam_role.lambda_exec.arn
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-token"
  }
}
...
resource "aws_lambda_function" "userinfo" {
  filename         = data.archive_file.lambda_zip.output_path
  function_name    = "${var.project_name}-${var.environment}-userinfo"
  role            = aws_iam_role.lambda_exec.arn
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-userinfo"
  }
}
